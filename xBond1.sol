...
contract PulseStrategy is ERC20, ReentrancyGuard {
    ...
    uint8 private constant _MIN_OUTPUT_PERCENT = 90; // 90% slippage tolerance
    ...
    uint256 private constant _TRADING_FEE = 9971; // 0.29% fee
    ...
    function _swapToPLSX(uint256 xBondAmount) private returns (uint256 plsxReceived) {
        ...
        if (reserveIn == 0 || reserveOut == 0 || reserveOut < _MIN_LIQUIDITY) revert InsufficientLiquidity();

        // Calculate expected output using standard AMM formula
        uint256 amountInAfterFee = (xBondAmount * _TRADING_FEE) / 1000; // Apply 0.3% trading fee
        uint256 amountOut = (amountInAfterFee * reserveOut) / (reserveIn + amountInAfterFee);

        // Apply slippage tolerance (90% of expected output)
        uint256 amountOutMin = (amountOut * uint256(_MIN_OUTPUT_PERCENT)) / 100;
        if (amountOutMin == 0) revert SwapFailed();
